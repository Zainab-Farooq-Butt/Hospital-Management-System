...
AddMedicineDialog::AddMedicineDialog(QWidget *parent) : QDialog(parent)
{
    ...
    txtDosage->setPlaceholderText("e.g. 500mg or 10ml");
    spinStock = new QSpinBox(this);
    spinStock->setRange(0, 100000);
    spinPrice = new QDoubleSpinBox(this);
    spinPrice->setRange(0, 1000000);
...
void AddMedicineDialog::onAdd()
{
    ...
    Medicine m;
    if (!m.isValidName(name.toStdString()))
    {
        QMessageBox::warning(this, "Invalid", "Invalid name.");
        return;
    }
    if (!m.isValidDosage(dosage.toStdString()))
    {
        QMessageBox::warning(this, "Invalid", "Invalid dosage.");
        return;
    }
    if (!m.isValidQuantity(stock))
    {
        QMessageBox::warning(this, "Invalid", "Invalid stock.");
        return;
    }
    if (!m.isValidPrice(price))
    {
...
void AddMedicineDialog::onAdd()
{
    ...
    if (!m.isValidName(name.toStdString()))
    {
        QMessageBox::warning(this, "Invalid", "Bad name.");
        return;
    }
    if (!m.isValidDosage(dosage.toStdString()))
    {
        QMessageBox::warning(this, "Invalid", "Bad dosage.");
        return;
    }
    if (!m.isValidQuantity(stock))
    {
        QMessageBox::warning(this, "Invalid", "Bad stock.");
        return;
    }
    if (!m.isValidPrice(price))
    {
        QMessageBox::warning(this, "Invalid", "Bad price.");
        return;
    }
