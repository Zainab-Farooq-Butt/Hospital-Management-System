    ...
    : QMainWindow(parent), currentUser(username)
{
    ...
    auto* bLO = mk("Logout");
    bLO->setStyleSheet("background: #ef4444; color: white;");
    ...
    QPixmap logo("C:/Users/mueez/.gemini/antigravity/brain/"
        "9baebc15-b0f9-41d5-b46c-d7303fba27d8/"
        "medical_logo_1778005812670.png");
    ...
    auto* subtitle = new QLabel(
        "Select an option from the sidebar to manage your hospital operations.",
        contentArea
    );
...
{
    AppointmentListDialog dlg(AppointmentListDialog::BY_DOCTOR,
        doctorId,
        this);
    dlg.exec();
}
...
{
    PersonalInfoDialog dlg(PersonalInfoDialog::DOCTOR_VIEW,
        loggedCNIC,
        this);
    dlg.exec();
}
    ...
    if (ok && !pid.isEmpty()) {
        ...
        if (!p.isValidPatientId(pid.toStdString())) {
            QMessageBox::warning(this, "Invalid",
                "Invalid Patient ID format (P-XXXX).");
            return;
        }

        if (!p.patientIdAlreadyExists(pid.toStdString(), "Patient.txt")) {
            QMessageBox::warning(this, "Not Found",
                "No patient exists with ID: " + pid);
            return;
        }

        MedicalRecordsViewDialog dlg(pid, this, doctorId);
        dlg.exec();
    }
}
...
{
    MedicalRecordAddDialog dlg(doctorId, this);
    dlg.exec();
}
