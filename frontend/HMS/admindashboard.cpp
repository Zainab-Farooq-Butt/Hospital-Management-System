...
AdminDashboard::AdminDashboard(const QString& username, QWidget* parent)
    ...
    auto* pageTitle = new QLabel("Patient Management", contentArea);
    ...
    auto* bUS = mkCardBtn("Update Staff Info", sPage);
    auto* bRD = mkCardBtn("Register Doctor", sPage);
    auto* bVD = mkCardBtn("Doctor Directory", sPage);
    auto* bUD = mkCardBtn("Update Doctor Info", sPage);
    auto* bDA = mkCardBtn("Manage Appointments", sPage);
    sGrid->addWidget(bRS, 0, 0); sGrid->addWidget(bVS, 0, 1); sGrid->addWidget(bUS, 0, 2);
    sGrid->addWidget(bRD, 1, 0); sGrid->addWidget(bVD, 1, 1); sGrid->addWidget(bUD, 1, 2);
    ...
    auto* bSPB = mkCardBtn("View Patient Bill", bPage);
    auto* bSETB = mkCardBtn("Set Patient Bill", bPage);
    auto* bSAB = mkCardBtn("All Bills", bPage);
    auto* bUPB = mkCardBtn("Update Bill", bPage);
    auto* bDP = mkCardBtn("Discharge Patient", bPage);
    auto* bAR = mkCardBtn("Room Assignment", bPage);

    // REORGANIZED GRID (2 Rows, 3 Columns)
    bGrid->addWidget(bSPB, 0, 0); bGrid->addWidget(bSETB, 0, 1); bGrid->addWidget(bSAB, 0, 2);
    bGrid->addWidget(bUPB, 1, 0); bGrid->addWidget(bDP, 1, 1);   bGrid->addWidget(bAR, 1, 2); // Moved up
    ...
    auto* bSPR = mkCardBtn("Search Records", phPage);
    ...
    phGrid->addWidget(bAPr, 2, 0); phGrid->addWidget(bSPR, 2, 1); phGrid->addWidget(bRM, 2, 2);
    ...
    connect(bSETB, &QPushButton::clicked, this, &AdminDashboard::onSetPatientBill);
...
void AdminDashboard::onViewMedicalRecords() {
    bool ok;
    QString pid = QInputDialog::getText(this, "Medical Records", "Enter Patient ID (P-XXXX):", QLineEdit::Normal, "", &ok);
    if (ok && !pid.isEmpty()) {
        Patient p;
        if (!p.isValidPatientId(pid.toStdString()) || !p.patientIdAlreadyExists(pid.toStdString(), "Patient.txt")) {
            QMessageBox::warning(this, "Error", "Patient ID invalid or not found.");
            return;
        }
...
void AdminDashboard::onUpdateStaff() { StaffUpdateDialog dlg(this); dlg.exec(); }
...
void AdminDashboard::onUpdateDoctor() { DoctorUpdateDialog dlg(this); dlg.exec(); }

void AdminDashboard::onViewDoctorAppointments() {
    bool ok;
    QString did = QInputDialog::getText(this, "Doctor Appointments", "Enter Doctor ID (D-XXXX):", QLineEdit::Normal, "", &ok);
    if (ok && !did.isEmpty()) {
        Doctor d;
        if (!d.isValidDoctorId(did.toStdString()) || !d.doctorIdAlreadyExists(did.toStdString(), "Doctor.txt")) {
            QMessageBox::warning(this, "Error", "Doctor ID invalid or not found.");
            return;
        }
...
void AdminDashboard::onSearchPharmacyRecords() {
    bool ok;
    QString pid = QInputDialog::getText(this, "Search Pharmacy", "Enter Patient ID (P-XXXX):", QLineEdit::Normal, "", &ok);
    if (ok && !pid.isEmpty()) {
...
void AdminDashboard::onUpdateAmbulance() { AmbulanceUpdateDialog dlg(this); dlg.exec(); }

void AdminDashboard::onRegisterAdmin() { AdminRegisterDialog dlg(this); dlg.exec(); }
