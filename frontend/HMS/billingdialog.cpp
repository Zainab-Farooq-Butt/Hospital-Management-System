...
void BillingDialog::onAction() {
    ...
    if (mode == SHOW_ONE) {
        ...
        QString summary = QString(
            ...
            .arg(doctorFee).arg(treatmentCost).arg(roomFee).arg(totalAmount);

        double amountReceived = QInputDialog::getDouble(this, "Payment",
            summary + "\n\nEnter Amount Received:", 0, 0, 1000000, 2, &ok);
        if (!ok) return;
        if (amountReceived <= 0) {
        ...
        // Check if patient has any bills (without using cout-based searchByPatientId)
        {
